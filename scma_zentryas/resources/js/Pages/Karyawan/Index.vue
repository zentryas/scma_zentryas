...
<template>
    <Head title="Manage Karyawan" />

    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Manage Karyawan</h2>
        </template>
                    ...
                    <div class="p-6 text-gray-900">
                        ...
                        <div class="relative overflow-x-auto sm:rounded-lg">
                            <table id="list_data" class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 nowrap">
                                ...
                                <tbody>
                                    <tr v-for="(key, index) in karyawan" :key="key.id">
                                      <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">
                                        {{ index + 1 }}.
                                      </th>
                                      <td class="px-6 py-4">{{ key.nama_karyawan }}</td>
                                      <td class="px-6 py-4">{{ key.nik }}</td>
                                      <td class="px-6 py-4">

                                        <Link :href="`karyawan/${key.id}/edit`">
                                        ...
                                        <DeleteButton 
                                          :item-id="key.id" 
                                          :item-nama_karyawan="key.nama_karyawan" 
                                          :delete-url="`/karyawan/${key.id}`"/>
                                      </td>
                                    </tr>
                                </tbody>
                            </table>
