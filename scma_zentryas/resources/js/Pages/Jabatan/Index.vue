...
<template>
    <Head title="Manage jabatan" />

    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Manage jabatan</h2>
        </template>
                            ...
                            <table id="list_data" class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 nowrap">
                                <thead>
                                    <tr>
                                        ...
                                        <th scope="col" class="px-6 py-3">Kode Jabatan</th>
                                        <th scope="col" class="px-6 py-3">Action</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <tr v-for="(key, index) in jabatan" :key="key.id">
                                      <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">
                                        {{ index + 1 }}.
                                      </th>
                                      <td class="px-6 py-4">{{ key.nama_jabatan }}</td>
                                      <td class="px-6 py-4">{{ key.kode_jabatan }}</td>
                                      <td class="px-6 py-4">

                                        <Link :href="`jabatan/${key.id}/edit`">
                                        ...
                                        <DeleteButton 
                                          :item-id="key.id" 
                                          :item-nama_jabatan="key.nama_jabatan" 
                                          :delete-url="`/jabatan/${key.id}`"/>
                                      </td>
                                    </tr>
                                </tbody>
                            </table>
